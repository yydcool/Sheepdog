...
public class Player extends sheepdog.sim.Player {
    ...
    public Point move(Point[] dogs, // positions of dogs
        ...
        if(mode && whiteOnLeft.length > 0 && sheeps.length==0){
            ...
            // White sheep on left
            whiteOnLeft = Util.sortWhiteOnLeft(whiteOnLeft); 

            // If white sheep on left < spare dogs
            if (countLazyDogs > 0 ) {

                // Allocate one white sheep to each dog
                if(whiteOnLeft.length>=id) {
                    sheep = whiteOnLeft[id - 1];
                }
                else
                    return dogs[id-1];
            ...
            Point dogPoint = positionDogNearLeftSheep(sheep, gate, 1.99);
  ...
  /*      if(mode && sheeps.length<dogs.length){
    ...
    static Point positionDogNearLeftSheep(Point from, Point to, double BUFFER) {
        ...
        if (finalDestination.x < 0)
        {
           ...
           finalDestination.y = slope*(0 - to.x) + to.y;
        }
